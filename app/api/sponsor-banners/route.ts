...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (!url || !name) {
            return NextResponse.json({ error: 'Both URL and banner name are required' }, { status: 400 });
        }
        ...
        const newBanner = await Advertisement.create({ url, name, category, status, link });
